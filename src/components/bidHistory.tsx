...
const BidHistory = () => {
    ...
    const { data, isLoading } = useGetBidHistoryQuery(user!)
    ...
    return (
        <div className='mt-16 md:w-6/12 w-10/12 mx-auto'>
            {isLoading ?
                <Skeleton width={'100%'} height={100} className='mx-auto'></Skeleton>
                : data !== 'doesNotExist' && data !== undefined ?
                    <table className='table-auto bg-orange-400 border rounded-sm shadow-lg'>
                        <thead className=' font-bold py-2 rounded-sm md:text-base text-sm'>
                            <tr>
                                <th><button onClick={() => handleSort('cost')} className="py-2 pl-6 pr-4 ">Cost <span className='md:text-xs text-[10px]'> (EUR/MWh)</span></button></th>
                                <th><button onClick={() => handleSort('quantity')} className="py-2 px-4"> Quantity <span className='md:text-xs text-[10px]'>(MWh)</span></button></th>
